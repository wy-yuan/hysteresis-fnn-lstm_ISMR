...
def train(args, model, device, train_loader, optimizer, model_name="LSTM", seg=50, f=True):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        if f:
            output, dynamic_weights, h_ = model(td, pre_td, h_init)    # for Bouc-Wen model
        else:
            ...
            output, dynamic_weights, h_ = model(tip, pre_tip, h_init)    # for Bouc-Wen model
...
def main():
    ...
    parser.add_argument('--model_name', type=str, default="BoucWen")
    parser.add_argument('--checkpoints_dir', type=str,
                        default="./checkpoints/Sinus_BoucWen_seg50_sr25_6w/") #stdRatio1e-4_delta1e-4
    ...
    filepath = "./tendon_data/Data_with_Initialization/Sinus"
    pos = 1
    seg = 50
    ...
    print('Training Bouc-Wen model.')
    model = BoucWenNet().to(device)
